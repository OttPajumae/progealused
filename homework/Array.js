//array 
let hinnad = [11,2,34,55,4,2,44,75,6,12,34,22,16]
let i = 0 
let min = 9999
let max = -9999
 
    while ( i <= hinnad.length) {
        i++
        if (hinnad[i] < min )
            min = hinnad[i]
...
while(i < offersArray.length) {
    if (offersArray[i][1] <= 20 ){ //[i] võtab terve kasti [i][number] võtab spetsiifilise omaduse kastist
        console.log(offersArray[i][1])
    ...
    //Saab ka offersObjects[i].hind:mõlemad töötavad
...
let i = 0
while (i < offersObjects.length) {
    if(offersObjects[i]["hind"] <= 20){ //Saab ka offersObjects[i].hind mõlemad töötavad
        console.log(offersObjects[i].pakkuja)
        console.log(offersObjects[i].hind)
    }
    i++
}

// fetch (js alla laadimine; vaata kui viitsid) 
